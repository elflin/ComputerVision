...
def draw_boxes(frame, v_boxes, v_labels, v_scores):
    ...
    #     label = "%s (%.1f)" % (v_labels[i], round(v_scores[i], 1))
    ...
    for i in range(len(v_boxes)):
        ...
        color = (0, 0, 255) 
        thickness = 2
        frame = cv2.putText(frame, v_labels[i]+": "+str(round(v_scores[i], 2)), start_point, font, fontScale, color, thickness, cv2.LINE_AA)
